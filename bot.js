const botFactory = (mode) => {
  ...
  const filterDublicates = (array) => [...new Set(array)];
  ...
  const plays = {
    impossible: () => {
      ...
      if (board.cells[5] == '') {
        ...
        else if (linesOneO.length) marksCell(linesOneX
          .filter(cell => [1, 3, 7, 9].includes(cell)))
        else marksCell(linesOneX
          .filter(cell => [2, 4, 6, 8].includes(cell)))

      } else if (board.cells[5] == 'X') marksCell(linesOneX
        .filter(cell => [1, 3, 7, 9].includes(cell)))
      else if (linesOneX.length == 6) marksCell(linesOneX
        .filter(cell => [2, 4, 6, 8].includes(cell)))
      else marksCell(linesOneX
        .filter(cell => [1, 3, 7, 9].includes(cell)));
      marksCell(linesOneX)
